...
def sanitize_image_profiles(data, kind):
    """Rewrite actual metadata containers, never search/replace image pixels."""
    if kind in {'HEIC', 'AVIF'}:
        return sanitize_bmff_profiles(data)
    if kind in {'PNG', 'APNG', 'BMP'}:
        if data[:8] != b'\x89PNG\r\n\x1a\n':
            raise PrivacyError('Invalid PNG signature')
        out, p, count = bytearray(data[:8]), 8, 0
        while p < len(data):
            if p + 12 > len(data):
                raise PrivacyError('Truncated PNG chunk')
            size, tag = struct.unpack_from('>I4s', data, p)
            end = p + size + 12
            if end > len(data):
                raise PrivacyError('Invalid PNG chunk length')
            payload = data[p + 8:end - 4]
            if tag == b'iCCP':
                zero = payload.find(b'\0')
                if not 1 <= zero <= 79 or payload[zero + 1:zero + 2] != b'\0':
                    raise PrivacyError('Invalid PNG color profile')
                try:
                    decoder = zlib.decompressobj()
                    profile = decoder.decompress(payload[zero + 2:], ICC_MAX_SIZE + 1)
                    if len(profile) > ICC_MAX_SIZE or not decoder.eof or decoder.unused_data:
                        raise PrivacyError('Invalid or oversized compressed ICC profile')
                except zlib.error as error:
                    raise PrivacyError('Invalid compressed ICC profile') from error
                payload = b'Clean\0\0' + zlib.compress(sanitize_icc(profile))
                out += struct.pack('>I', len(payload)) + tag + payload + struct.pack('>I', zlib.crc32(tag + payload))
                count += 1
            else:
                out += data[p:end]
            p = end
    ...
    if kind == 'WEBP':
        if data[:4] != b'RIFF' or data[8:12] != b'WEBP':
            raise PrivacyError('Invalid WebP signature')
        out, p, count = bytearray(data), 12, 0
        while p < len(data):
            if p + 8 > len(data):
                raise PrivacyError('Truncated WebP chunk')
            tag, size = struct.unpack_from('<4sI', data, p)
            end = p + 8 + size
            if end > len(data):
                raise PrivacyError('Invalid WebP chunk length')
            if tag == b'ICCP':
                out[p + 8:end] = sanitize_icc(bytes(data[p + 8:end]))
                count += 1
            p = end + size % 2
        return bytes(out), count
    if kind == 'TIFF':
        return sanitize_tiff_profiles(data)
    if kind == 'GIF':
        return sanitize_gif_profiles(data)
    if kind == 'JPEG':
        return sanitize_jpeg_profiles(data)
    raise PrivacyError('Unsupported color profile container: ' + kind)


def sanitize_jpeg_profiles(data):
    out = bytearray(data)
    p, pieces, total = 2, {}, None
    while p < len(data):
        if data[p] != 255:
            raise PrivacyError('Invalid JPEG profile container')
        while data[p] == 255:
            p += 1
        tag = data[p]
        p += 1
        if tag in {0xd9, 0xda}:
            break
        if tag == 1 or 0xd0 <= tag <= 0xd7:
            continue
        size = int.from_bytes(data[p:p + 2], 'big')
        end = p + size
        if size < 2 or end > len(data):
            raise PrivacyError('Invalid JPEG profile segment')
        if tag == 0xe2 and data[p + 2:p + 14] == b'ICC_PROFILE\0':
            index, declared = data[p + 14:p + 16]
            if not 1 <= index <= declared or index in pieces or total not in {None, declared}:
                raise PrivacyError('Invalid multipart JPEG ICC profile')
            total = declared
            pieces[index] = (p + 16, end)
        p = end
    if not pieces:
        return data, 0
    if set(pieces) != set(range(1, total + 1)):
        raise PrivacyError('Incomplete JPEG ICC profile')
    profile = b''.join(data[a:b] for _, (a, b) in sorted(pieces.items()))
    clean, p = sanitize_icc(profile), 0
    for _, (a, b) in sorted(pieces.items()):
        out[a:b] = clean[p:p + b - a]
        p += b - a
    return bytes(out), 1


def sanitize_tiff_profiles(data):
    if data[:2] not in {b'II', b'MM'}:
        raise PrivacyError('Invalid TIFF byte order')
    order = '<' if data[:2] == b'II' else '>'
    magic, = struct.unpack_from(order + 'H', data, 2)
    big = magic == 43
    if magic not in {42, 43} or (big and data[4:8] != struct.pack(order + 'HH', 8, 0)):
        raise PrivacyError('Unsupported TIFF header')
    width, entry_size, count_width = (8, 20, 8) if big else (4, 12, 2)
    out, visited, count = bytearray(data), set(), 0

    def integer(p, size):
        if p < 0 or p + size > len(data):
            raise PrivacyError('TIFF metadata offset out of bounds')
        return int.from_bytes(data[p:p + size], 'little' if order == '<' else 'big')

    queue = [integer(8 if big else 4, width)]
    while queue:
        pointer = queue.pop()
        if not pointer or pointer in visited:
            continue
        visited.add(pointer)
        n = integer(pointer, count_width)
        if pointer + count_width + n * entry_size + width > len(data):
            raise PrivacyError('Truncated TIFF directory')
        for i in range(n):
            p = pointer + count_width + entry_size * i
            tag, typ = integer(p, 2), integer(p + 2, 2)
            length = integer(p + 4, width)
            value = p + 4 + width
            if tag == 34675:
                if typ not in {1, 7} or length <= width:
                    raise PrivacyError('Invalid TIFF ICC profile')
                start = integer(value, width)
                if start + length > len(data):
                    raise PrivacyError('TIFF ICC profile out of bounds')
                out[start:start + length] = sanitize_icc(bytes(data[start:start + length]))
                count += 1
            elif tag in {330, 34665, 34853, 40965}:
                size = {4: 4, 13: 4, 16: 8, 18: 8}.get(typ)
                if size is None:
                    raise PrivacyError('Unsupported TIFF directory pointer')
                start = value if length * size <= width else integer(value, width)
                queue.extend(integer(start + i * size, size) for i in range(length))
        queue.append(integer(pointer + count_width + n * entry_size, width))
    return bytes(out), count


def sanitize_gif_profiles(data):
    if data[:6] not in {b'GIF87a', b'GIF89a'} or len(data) < 13:
        raise PrivacyError('Invalid GIF header')
    out, count = bytearray(data), 0
    p = 13 + (3 * (2 ** ((data[10] & 7) + 1)) if data[10] & 128 else 0)
    ...
    def subblocks(p):
        ...
        while p < len(data):
            ...
            if p + size > len(data):
                break
            pieces.append((p, p + size))
            p += size
        raise PrivacyError('Truncated GIF data block')
    ...
    while p < len(data):
        ...
        if tag == 0x2c:
            if p + 9 > len(data):
                raise PrivacyError('Truncated GIF image')
            flags = data[p + 8]
            p += 9 + (3 * (2 ** ((flags & 7) + 1)) if flags & 128 else 0) + 1
            _, p = subblocks(p)
        elif tag == 0x21:
            label = data[p]
            p += 1
            pieces, p = subblocks(p)
            if label == 0xff and pieces and data[pieces[0][0]:pieces[0][1]] == b'ICCRGBG1012':
                spans = pieces[1:]
                profile = b''.join(data[a:b] for a, b in spans)
                clean, index = sanitize_icc(profile), 0
                for a, b in spans:
                    out[a:b] = clean[index:index + b - a]
                    index += b - a
                count += 1
        else:
            raise PrivacyError('Unknown GIF block')
    return bytes(out), count
